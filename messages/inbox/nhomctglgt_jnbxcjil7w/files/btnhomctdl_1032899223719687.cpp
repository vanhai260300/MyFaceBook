...
void display(Node_BnTr *Tr)
{
	...
	{
		if(Tr->dtb>=5.5)
			printf("\n%s|%s|%s|%0.1f",Tr->masv,Tr->hotensv,Tr->lop,Tr->dtb);
		display(Tr->Adr_TrR);
		display(Tr->Adr_TrL);
	}
}
...
main()
{
	...
	while(choose!=5)
	{
		...
		printf("\n3.Xoa trong cay nhi phan nhung sinh vien co diem trung binh duoi 5.5: ");
		printf("\n4.Bo sung sinh vien: ");
		printf("\n5.Thoat");
		printf("\nMoi nhap lua chon: ");
		scanf("%d",&choose);
